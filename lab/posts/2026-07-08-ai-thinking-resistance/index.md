---
title: "不是 AI 讓學生變笨，是我們太快收下漂亮答案"
author: "張修瑋"
date: "2026-07-08"
categories:
  - AI 教學與研究
  - AI教育
  - 教學實踐
  - 學習方法
description: "AI 進入課堂後，真正該被重新設計的不是禁止規則，而是作業如何逼學生留下判斷、錯誤與修正痕跡。"
image: thinking-trace.png
draft: false
---

![AI 作業的思考痕跡：學生先留下原始判斷，再讓 AI 反駁，最後附上查證過程與修正說明。](thinking-trace.png)

學生可以錯，但不能空著手進入工具。

## 太乾淨的答案，常常沒有主人

有一種作業，我們一看就知道不對勁。段落完整，語氣平穩，轉折乾淨，沒有錯字，也沒有任何猶豫。它讀起來像答案，卻不像一個學生剛想完問題後留下的文字。過去我們會把這種東西稱為「寫得很好」。現在我們反而要停一下。太乾淨，有時不是成熟，而是沒有經過人。

我不太接受「AI 會讓學生變笨」這句話。它太方便了。工具被推到前面，題目本身就不用被檢查。可是很多作業原本就沒有要求學生留下任何非他不可的東西：請說明某概念、請整理某文章、請提出你的看法。這些題目在沒有 AI 的年代就已經很薄，只是以前學生要花時間把薄的東西寫出來。現在模型幾秒鐘就能把它包裝好。

真正的問題不是學生會不會按下送出，而是我們的作業有沒有要求他先站出來。沒有原始判斷的答案，不管多流暢，都只是一個輸出。它可能正確，可能漂亮，可能足以通過評分表，但它沒有讓學生冒任何思想上的風險。教育最怕的不是學生犯錯，而是整份作業裡沒有一處需要他承擔判斷。

我會把這種風險稱為「第一筆押注」。學生不需要一開始就對，但他要先押一個方向。沒有押注，後面的修正就只是文字加工；有了押注，AI 的反駁才會真的碰到他。學習有時不是多得到一段解釋，而是發現自己原本相信的東西站不住。

第一筆押注不該被拿來羞辱學生。這一點很重要。學生願意留下粗糙判斷，前提是他知道粗糙不會直接變成扣分證據。教師要把第一版當作起點，而不是把它拿來證明學生不夠好。若課堂只獎勵最後的漂亮答案，學生當然會把前面的笨拙藏起來。現在的作業設計，必須替笨拙留一個安全位置。

## 先交出粗糙，才有資格請 AI 幫忙

我會把 AI 使用規則改得很簡單：學生可以用 AI，但必須先交出第一版判斷。那一版可以短，可以笨，可以有錯。它甚至可以只有幾句：「我目前猜這家公司營收成長不是好事，因為應收帳款也升高，但我還沒有檢查現金流。」這種句子不漂亮，卻有一個很重要的東西：它有主人。

接著才讓 AI 進場。不是請它代寫，而是請它反駁、追問、提出相反解釋。學生最後要交的，也不是單純修正版，而是三個小註記：哪一點被 AI 說服，哪一點沒有接受，哪一點回去查了資料。這三個註記比最後那篇順滑文章更值得看，因為它們讓我們知道學生如何移動。

![概念圖把 AI 使用分成兩種做法：直接外包會削弱思考，先形成判準再使用 AI，才會放大學習。](concept.png)

工具不是起點。工具應該碰到一個已經有方向、也可能犯錯的人。

這裡最困難的地方，不在學生，而在教師。我們過去太習慣批改成果，現在卻要回頭看形成。草稿、提問、查證過程、被反駁後的修改，這些東西比最後答案亂，也更難評分。可是如果最後答案已經變得廉價，我們還只看最後答案，就是假裝教室沒有改變。

因此，評分表也要改。不能只看最後文章是否完整，也要看第一版判斷是否具體，AI 反駁是否真的被處理，查證是否回到可靠材料，修正版是否說明自己改了哪裡。這些標準比較麻煩，但它們逼學生留下移動的痕跡。沒有移動，就算最後答案正確，也不一定代表學習發生。

**評分要獎勵改變，而不是只獎勵乾淨**

我會在評分表裡放一個很小的欄位，叫「我原本怎麼想，後來改了哪裡」。這個欄位不需要長，三句就夠。第一句寫原判斷，第二句寫 AI 或資料如何挑戰它，第三句寫最後如何修正。這三句比一段漂亮結論更難偽裝，因為它要求學生承認自己移動過。課堂若只獎勵乾淨，學生就會把所有移動痕跡擦掉。

## 粉筆不能交出去

我喜歡把這件事想成黑板。學生先拿粉筆寫下一個解法。旁邊有人提醒他少算了一個條件，有人問他資料在哪裡，有人提出另一種可能。AI 只是那個旁邊的人之一。它可以很快，可以很耐心，可以一次提出十種反例，但粉筆不能交到它手上。

一旦粉筆交出去，學生就從思考者變成審稿者，而且常常審得很鬆。模型寫出來的文字太像答案，學生很容易只做表面修改。換幾個詞，刪幾句太像機器的話，交出去。這不是使用 AI，這是替 AI 做排版。好的作業設計要讓學生知道：AI 可以坐在旁邊，但第一筆與最後一筆都要由人寫下。

![AI 時代的作業契約：容易被外包的題目只問最後答案，逼學生出場的題目要求課堂脈絡、原始錯誤與 AI 對話摘要。](assignment-contract.png)

作業規則不是禁止工具，而是要求留下判斷。

因此，題目要把人的位置寫進去。不要只問「請說明某概念」，而要問「請用本週課堂中你原本聽錯的一個地方，重新說明這個概念」。不要只問「請整理文章重點」，而要問「請指出你原本最想同意的一句話，後來為什麼改變或沒有改變」。不要只問「請提出看法」，而要問「請附上 AI 反駁你的三句話，並說明你接受哪一句、拒絕哪一句」。

這些限制不花俏，但它們會提高代寫成本。更重要的是，它們讓學生知道，課堂在乎的不是文件完成，而是判斷有沒有被留下來。

我也會安排一種很小的課堂活動：每個學生只交一句原始判斷，不交完整文章。老師抽幾句匿名投影，請全班判斷哪一句最容易被 AI 包裝，哪一句最有修正價值。學生會發現，模糊句子最容易被模型接管；具體句子比較不漂亮，但比較能被追問。這一課比講一小時學術誠信還有效。

這個活動的重點不是抓誰比較會寫，而是讓全班看見句子的責任密度。「這家公司表現不錯」很安全，也很空；「營收增加但應收帳款增加更快，所以我懷疑現金回收有壓力」比較冒險，也比較有學習價值。AI 可以把第一句變成一段漂亮文章，卻很難替第二句承擔後面的查證。句子一旦有責任密度，學生就比較不容易把自己交出去。

## 偵測工具不能替我們看見學習

AI 偵測工具給人一種安全感。它丟出百分比，好像問題可以被量化。可是學生是否思考，不能靠一個機率決定。真正可檢查的是過程：他一開始怎麼想？他問了什麼？他怎麼改？他留下哪些資料來源？這些東西麻煩、笨重、不漂亮，但比偵測分數更誠實。

我們不應該把課堂變成追查警察。那會讓學生學會隱藏，而不是學會判斷。更誠實的做法，是公開承認 AI 會進入學習，然後把成熟的使用方式說清楚。哪一步可以用，哪一步不能用，哪一步要附上紀錄。學生不會因為規則存在就成熟，但至少他知道成熟不是把工具藏起來，而是能說明自己怎麼用。

我也不會要求學生把完整對話紀錄全部交上來。那會把作業變成資料垃圾場，也會讓學生以為透明就是把所有東西倒出來。比較好的做法，是交一小段關鍵對話：AI 哪一句改變了你的想法，哪一句你拒絕了，哪一句讓你回去查資料。教師要看的不是聊天紀錄的長度，而是學生有沒有在對話裡做判斷。

這也讓教師少一點道德戲劇，多一點教學設計。與其問學生有沒有偷用 AI，不如問：你的第一版判斷在哪裡？你的查證過程在哪裡？你原本錯在哪裡？這些問題更慢，卻更接近學習。

這種設計也比較公平。擅長寫作的學生過去可以用流暢文字遮住薄弱思考；不擅長寫作的學生可能有好的判斷，卻輸在表達。當我們要求第一版判斷、查證過程與修正理由，評量看的就不只是文字表面，而是學生如何處理自己的不確定。

## 讓學生練習和答案保持距離

AI 最危險的地方不是錯，而是它錯得很順。它能把沒有根據的話寫得像有根據，把模糊的判斷寫得像成熟結論。學生若沒有練習和文字保持距離，很容易被語氣帶走。這正是我們應該教的能力：看見一段流暢文字，先不要臣服。

我會在課堂上示範自己怎麼拆 AI 的答案。不要只拿修好的版本給學生看。把失敗版本投影出來，讓學生看見：這一句偷換定義，這一句沒有來源，這一句語氣太滿，這一句看似合理但還不能採用。學生需要看到成年人如何面對一段很會說話但不可靠的文字。

示範時，我會故意保留自己的猶豫。比如我會說：「這句我現在還不能判斷，因為我還沒看到來源。」學生需要看到老師不是永遠立刻知道答案的人，而是願意停下來查的人。AI 時代最值得教的姿態，也許不是更快回答，而是更有紀律地暫停。

最後，考試與平時作業也不必互相否定。平時訓練學生帶著工具思考，考試檢查他把工具蓋上後還剩下什麼。這個標準比禁止更高。禁止只要求服從；這個標準要求學生真的有東西可說。

AI 給出的第一個答案，不是終點。它是一個需要被審問的對象。學生若能學會這一點，他不會因為 AI 變笨。相反地，他會開始明白：思考不是把答案生出來，而是知道答案出現後，自己還不能停。

所以我不會把「少用 AI」當成課堂目標。真正的目標是讓學生用完 AI 之後，還能指出哪一句是自己願意負責的。若他只能說「AI 這樣寫」，那他還在外包。若他能說「我刪掉這句、保留那句，因為資料只能支持到這裡」，他才開始把思考拿回來。
