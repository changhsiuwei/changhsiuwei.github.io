...
可是這個做法也有危險。若學生只把一句「幫我們做財報分析」丟給 AI，得到的東西再像樣，也只是責任外包。Vibe Coding 進會計課，第一步不是寫提示詞，而是寫需求合約。
...
![Vibe Coding 需求合約圖：白話需求先拆成資料、例外與查核三個層次。](requirement-contract.png)
...
需求合約還要寫出「不做什麼」。例如這次只抓月營收，不抓股價；只處理公開資訊觀測站資料，不混進二手整理網站；只輸出 Excel，不自動產生投資結論。限制寫得越清楚，AI 越不容易替學生加戲。會計工作裡，知道邊界有時比知道功能更重要。

**需求合約要有人願意署名**

我會要求每一組在需求合約最後寫上負責人，不是為了形式，而是讓學生知道這份需求會被別人照著執行。只要有人署名，句子就會變得比較小心。「抓財報」會變成「抓合併資產負債表、損益表與現金流量表，期間為近五年，單位為新台幣千元」。署名不是行政動作，它會讓模糊變得不好意思。
...
錯誤也要分類。環境錯誤、資料格式錯誤、欄位定義錯誤、網站結構改變、需求說不清楚，這些不是同一種錯。若學生只寫「已修正」，那還是把錯誤當成障礙；若他能說出錯誤是哪一類，他就開始知道下次要先檢查哪裡。程式課最有價值的地方，往往不在順利跑完，而在學生第一次能替錯誤命名。

錯誤分類也會讓學生看見自己的責任。有些紅字是環境造成的，有些紅字是資料來源變了，有些紅字其實是需求寫壞了。第三種最刺人，因為它不是電腦不聽話，而是我們沒有把工作交代清楚。會計學生若能承認這一點，程式課就不只是技術訓練，而是在練習把責任說清楚。
...
資料卡也可以讓另一組同學驗收。驗收的人不看程式碼，只看資料卡和輸出檔。他要能回答：這份資料從哪裡來？單位是什麼？哪幾列被人工抽查？哪些欄位不能直接拿來推論？如果驗收者答不出來，問題不一定在驗收者，可能是資料卡沒有把責任寫清楚。

我會把驗收設計成一種同儕審查。寫程式的人常常太熟自己的檔案，會自動補上很多沒有寫出來的脈絡。另一組同學看不懂，正好能暴露文件缺口。這不是找碴，而是模擬真實工作：報表會被交給沒有參與開發的人，資料會被拿去開會，錯誤會在作者不在場時發生。文件若不能讓陌生人接手，就還不算完成。
...
交付條件要很具體。不是「可以使用」，而是另一組同學能在五分鐘內依說明重跑，能找到輸出檔，能知道資料日期，能看見錯誤訊息，能回報哪一步失敗。這些條件看起來瑣碎，但它們把作品從個人作業變成可接手的工作。會計資訊系統最需要的不是天才，而是別人敢接手。

這一點我會講得很白：不能交接的工具，不是工具，只是作者本人的延伸。學生做出一個自己會用的爬蟲，當然值得肯定；但若同學不能重跑、不能查資料日期、不能理解錯誤訊息，那它還沒有進入組織。會計資訊系統的價值，不在於某個人很會操作，而在於人換了，工作還能繼續。
...
如果學生最後只記得「AI 幫我寫出爬蟲」，這堂課就太薄了。比較好的結果，是他記得自己如何把一句模糊需求改成合約，如何把紅字改成錯誤分類，如何把資料圖改成資料卡，如何讓同學重跑自己的工具。程式只是外殼，裡面真正被訓練的是把工作交代清楚的能力。
