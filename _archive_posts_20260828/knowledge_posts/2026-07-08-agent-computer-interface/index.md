...
一個人類介面常常把責任藏在默契裡。使用者知道哪裡要小心，知道哪個按鈕不能亂按，知道跳出警告時該停一下。Agent 沒有這些社會經驗。若系統沒有把責任寫成狀態、權限和錯誤回報，模型就只能用猜的。這不是模型的錯，是系統把責任藏得太深。
...
以網站管理為例，人類編輯需要好的後台。可以看文章、改標題、插圖片、預覽、同步。這些畫面值得打磨。可是 Agent 不應該只靠這個畫面工作。它更適合走另一套流程：讀文章清單，取得單篇原始內容，寫入草稿，執行 render，檢查圖片，提交版本，推送部署，再輪詢正式頁面。

這套流程可以沒有按鈕，但不能沒有狀態。

我會把這套流程想成後台的第二個門。第一個門給人進去，有畫面、有預覽、有可以慢慢看的脈絡。第二個門給 Agent 進去，有明確命令、有版本、有狀態、有檢查。兩個門通往同一個系統，但不能混成同一個入口。混在一起，最後人要遷就機器，機器也要猜人的畫面。

第二個門也不能變成後門。它要比人類後台更嚴格，而不是更鬆。每一個命令要知道誰呼叫、改了什麼、能不能復原、要不要核准。若 Agent 的入口繞過了人類後台裡原本的安全設計，那不是自動化，是把風險藏到比較看不見的地方。

## Headless 多出一套可重跑流程

很多人聽到 headless，會以為只是把畫面拿掉。不是。Headless 的重點不是少一個畫面，而是多一套穩定流程。
...
每個命令都要說清楚自己回報什麼。`save_draft` 不能只回「完成」，它要回檔案路徑、修改時間、是否碰到圖片、是否更新索引。`verify_live_page` 不能只回「正常」，它要回它查了哪個網址、找到哪個標題、頁面裡有哪些圖檔、是否還停在舊版本。
...
狀態還要有主人。draft 是誰標的？published 是誰核准的？failed 是哪一個檢查判定的？如果狀態只是系統裡的一個字，沒有人能追問，它仍然不夠。真正能維護的系統，狀態背後要接得上人、規則和紀錄。
...
![Feature flag 把新能力放進可復原的迴圈](feature-flag-loop.png)

Feature flag 還有一個好處：它讓比較變得可能。舊流程和新流程可以同時跑，同一批文章跑兩套，再看哪一套錯少、哪一套省時、哪一套需要人改得少。沒有比較，大家只能憑感覺吵；憑感覺談 Agent，很快就會變成信仰問題。

新能力也要有復原機制。Agent 可以改一篇文章，就必須能說出改了哪裡；可以發布，就必須能回到上一版；可以批次處理，就必須能停在第幾筆。不能復原，就不該叫自動化，只能叫賭運氣。辦公室裡真正能用的系統，不是永遠不錯，而是錯了能退回來。
...
這些紀錄會變成下一次改進的材料。錄影只能告訴我們它當時看起來怎樣，執行紀錄才能告訴我們系統哪裡少了一個檢查、哪裡少了一個狀態、哪裡少了一個更好的命令。
...
讓 Agent 進系統，不是讓它更像人，而是讓系統比較像可以被交接的工作。命令清楚、狀態清楚、錯誤清楚、紀錄清楚，人和 Agent 才能各自站在對的位置。若所有事情都靠畫面猜，Agent 再聰明，也只是在替我們暴露系統本來就沒有說清楚的地方。

所以 ACI 最後不是介面美學，而是責任工程。它問的不是畫面漂不漂亮，而是工作能不能被說清楚、被重跑、被追問、被交接。Agent 進來後，系統裡那些原本靠人類默契撐住的地方，會一個一個露出來。這也許很尷尬，但尷尬正是改系統的入口。
