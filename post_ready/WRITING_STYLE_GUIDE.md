...
- TikZ 繪圖規範：
  ...
  - 呼吸空間限制式：框與框不能只是「沒有重疊」，而要有明顯空氣感。一般卡片之間至少留 0.45cm，主要面板之間至少留 0.7cm；上下堆疊的清單列，列中心距離至少 0.9cm。若畫面縮成文章寬度後看起來像黏在一起，就算 TikZ 沒報錯也要重排。
  - 內距限制式：一般文字框 `inner xsep` 不低於 10pt、`inner ysep` 不低於 8pt；兩行文字或中文字較多時，`inner ysep` 不低於 10pt。大外框內側到第一個物件至少留 0.45cm，底部說明到外框至少留 0.35cm。
  - 留白與排版：文字周圍必須有空氣感。標籤絕對不能碰到邊框、箭頭或其他物件。箭頭不能穿過文字，物件不能互相重疊。文字框不要填滿。徽章、狀態標籤、過/擋/通過等小物件不能貼著主清單或外框，必須獨立成可辨識的物件。
...
- 每張 TikZ 圖都通過呼吸空間檢查：框與框有明顯間距，文字沒有貼邊，狀態徽章沒有擠在清單框上。
- 絕對沒有使用任何 Emoji。
- 中英文與數字之間已確實加上半形空格。
